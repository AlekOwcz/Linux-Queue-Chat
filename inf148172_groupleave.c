...
void groupleave(int* LOGGEDIN, int *ID, int msgFromServer, int clientServerCom){
    if(*LOGGEDIN == 0){
    ...
    while(1){
        ...
        if(!strcmp(groupName, currentGroupName)){
            ...
            if(c == '0'){
                printf("@You are not a part of this group already!\n");
                close(fd);
                return;
            }
            if(c == '1'){
                ...
                if(conf.confirmation == 0) {
                    printf("@Error: Couldn't leave group!\n");
                    lseek(fd, -1, SEEK_CUR);
                    newMem = '1';
                    write(fd, &newMem, 1);
                } else printf("@Success!\n");
